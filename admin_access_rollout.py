...
def grant_admin_access():
    ...
    This function retrieves the current user ID, checks if the user already has admin access,
    and grants admin access if not. It updates the user access levels and saves them to the file.
    """
    ...
    if current_user_id in user_access_levels:
        ...
        if access_level == "admin":
            messagebox.showinfo("Info", f"User {current_user_id} already has admin access.")
            return
    register_or_update_user(current_user_id, "admin")
    user_access_levels = load_user_access_levels()
    save_user_access_levels(user_access_levels)
    messagebox.showinfo("Success", f"Granted admin access to user: {current_user_id}")
